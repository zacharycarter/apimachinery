...
type KubedbInterface interface {
...
type KubedbClient struct {
	restClient rest.Interface
}

func (c *KubedbClient) DormantDatabases(namespace string) DormantDatabaseInterface {
	return newDormantDatabases(c, namespace)
}

func (c *KubedbClient) Elasticsearchs(namespace string) ElasticsearchInterface {
...
func (c *KubedbClient) Postgreses(namespace string) PostgresInterface {
...
func NewForConfig(c *rest.Config) (*KubedbClient, error) {
	config := *c
	...
	return &KubedbClient{client}, nil
}

// NewForConfigOrDie creates a new KubedbClient for the given config and
// panics if there is an error in the config.
func NewForConfigOrDie(c *rest.Config) *KubedbClient {
...
// New creates a new KubedbClient for the given RESTClient.
func New(c rest.Interface) *KubedbClient {
	return &KubedbClient{c}
}

func setConfigDefaults(config *rest.Config) error {
	g, err := scheme.Registry.Group("kubedb.com")
	if err != nil {
		return err
	}
...
func (c *KubedbClient) RESTClient() rest.Interface {
	if c == nil {
		return nil
	}
	return c.restClient
